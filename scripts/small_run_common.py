...
def resolve_dataset_root(user_root: str) -> Path:
    ...
    Datasets live in the sibling repository at ../facexformer-my/datasets
    relative to facexformer-main. Pass that path (or override via --dataset-root).
    If the given root contains a nested datasets/ subfolder with 300w/ inside,
    that nested folder is preferred (handles legacy copied-repo layouts).
    """
