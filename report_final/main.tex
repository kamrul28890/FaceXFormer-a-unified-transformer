...
\begin{abstract}
FaceXFormer proposes a unified, real-time transformer for multi-task facial analysis. While the original release provides pretrained weights, the absence of the training pipeline limits independent reproducibility. This report reconstructs the end-to-end training and evaluation framework for eight face analysis tasks: parsing, landmark detection, head pose estimation, attribute prediction, age estimation, gender classification, race classification, and visibility prediction. We implemented task-aware preprocessing, an upsampling-based multi-task sampler, and a staged co-training protocol. Evaluating our trained model on standard benchmarks demonstrates strong performance on dense and binary classification tasks, matching the paper's target within 1.37\% for CelebAMask-HQ segmentation (90.64\% F1) and 0.46\% for CelebA attribute prediction (91.37\% accuracy). However, we observe notable degradation in continuous regression tasks, specifically head pose (21.80 MAE) and age estimation (10.74 MAE), highlighting the sensitivity of multi-task representation learning to undocumented architectural and loss-scaling divergences. Ultimately, we demonstrate that while FaceXFormer's core decoder architecture is effective for classification, full multi-task reproducibility requires exact specification of the joint optimization framework.
\end{abstract}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/fig1_facexformer_pipeline.pdf}
...
Landmark Detection & 68 tokens, one per keypoint & 1 token -> MLP -> 136 coordinates & \textbf{HIGH - fundamentally different tokenization} \\
Head Pose Estimation & 9 tokens for a 3x3 rotation matrix & 1 token -> MLP -> 3 Euler angles & \textbf{HIGH - different representation and output geometry} \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{assets/fig2_facex_decoder_block.pdf}
...
Landmark & Hourglass network on 68 landmark tokens & MLP on 1 landmark token -> 136 coordinates & STARLoss-style landmark loss & \textbf{HIGH} \\
Head Pose & Regression MLP from 9 rotation-matrix tokens & MLP on 1 pose token -> 3 Euler angles & Geodesic loss via Euler-to-rotation conversion & \textbf{HIGH} \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/fig8_gap_analysis_heatmap.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{assets/fig3_staged_training_timeline.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/fig4_baseline_inference_bars.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/fig5_training_results_flagged.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{assets/fig7_loss_scale_comparison.pdf}
...
\midrule
\endhead
Fig. 3.1 & FaceXFormer end-to-end pipeline & \path{assets/fig1_facexformer_pipeline.pdf}; redrawn from paper architecture description \\
Fig. 3.2 & FaceX decoder block detail & \path{assets/fig2_facex_decoder_block.pdf}; drawn from TSA, TFCA, FTCA decoder description \\
Fig. 4.1 & Gap analysis summary heatmap & \path{assets/fig8_gap_analysis_heatmap.pdf}; generated from Table 4.1 impact labels \\
Fig. 4.2 & Staged training strategy timeline & \path{assets/fig3_staged_training_timeline.pdf}; generated from staged 3-to-6-to-8 task schedule \\
Fig. 5.1 & Baseline inference results bar chart & \path{assets/fig4_baseline_inference_bars.pdf}; generated from fixed baseline JSON \\
Fig. 6.1 & Preliminary training results with flagged rows & \path{assets/fig5_training_results_flagged.pdf}; generated from report instruction/final presentation values \\
Fig. 7.1 & Loss scale comparison across tasks & \path{assets/fig7_loss_scale_comparison.pdf}; illustrative loss-scale comparison \\
\bottomrule
\end{longtable}
\endgroup
